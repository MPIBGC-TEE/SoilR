...
#' data.frame(Month=month.name,Rainfall=P,
#' Evaporation=E, pE= (E*0.75), SMD=(P-0.75*E),Rothamsted)
#' 
fW.RothC<-function(
     P,           
     E,           
    ...
    {  
     ...
     Acc.TSMD=NULL
       for(i in 2:length(M)){
          Acc.TSMD[1]=ifelse(M[1] > 0, 0, Max.TSMD)
     ...
     b=ifelse(test=Acc.TSMD > 0.444*Max.TSMD, # test uses < instead of > because values are negative
              yes=1,
              no=(0.2+0.8*((Max.TSMD-Acc.TSMD)/(Max.TSMD-0.444*Max.TSMD)))
              )
