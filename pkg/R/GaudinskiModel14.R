...
GaudinskiModel14<- function #Implementation of a the six-pool C14 model proposed by Gaudinski et al. 2000
  ...
  (t,      ##<< A vector containing the points in time where the solution is sought. It must be specified within the same period for which the Delta 14 C of the atmosphere is provided. The default period in the provided dataset \code{\link{C14Atm_NH}} is 1900-2010.
   ...
   solver=deSolve.lsoda.wrapper, ##<< A function that solves the system of ODEs. An alternative to the default is  \code{\link{euler}} or  any other user provided function with the same interface.
