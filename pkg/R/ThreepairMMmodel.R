...
#' 
#' This function implements a 6-pool Michaelis-Meneten model with pairs of
#' microbial biomass and substrate pools.
#' 
#' 
#' @param t vector of times to calculate a solution.
#' @param ks a vector of length 3 representing SOM decomposition rate (m3 d-1
#' (gCB)-1)
#' @param kb a vector of length 3 representing microbial decay rate (d-1)
#' @param Km a vector of length 3 representing the Michaelis constant (g m-3)
#' @param r a vector of length 3 representing the respired carbon fraction
#' (unitless)
#' @param Af a scalar representing the Activity factor; i.e. a temperature and
#' moisture modifier (unitless)
#' @param ADD a vector of length 3 representing the annual C input to the soil
#' (g m-3 d-1)
#' @param ival a vector of length 6 with the initial values of the SOM pools
#' and the microbial biomass pools (g m-3)
