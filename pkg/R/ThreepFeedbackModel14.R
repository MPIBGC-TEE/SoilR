...
#' is a wrapper for the more general function \code{\link{GeneralModel_14}}
#' that can handle an arbitrary number of pools with arbitrary connections.
#' \code{\link{GeneralModel_14}} can also handle input data in different
#' formats, while this function requires its input as Delta14C. Look at it as
#' an example how to use the more powerful tool \code{\link{GeneralModel_14}}
...
#' the 3 pools.
#' @param F0_Delta14C A vector of length 3 containing the initial fraction of
#' radiocarbon for the 3 pools in Delta14C format. The format will be assumed
#' to be Delta14C, so please take care that it is.
...
#' 
#' par(mfrow=c(1,1))
ThreepFeedbackModel14<- function 
  (t,      
   ks,	
   C0,	
   F0_Delta14C,  
