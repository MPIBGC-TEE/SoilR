...
%@
% 
% Notice that our function, {\tt eCO2func}, requires a (for this first example one dimensional) vector of parameters {\tt pars} with the values of the first decomposition rate in positions 1. Our function returns a {\tt data.frame} with two columns, time in days and the sum of the cummulative release for the two pools. 
% 
...
%For conveniance we use the \FME function {\tt modCost} which returns an object of a class with the same name. 
%<<>>=
...
%summary(eCO2P1afit)
%@
%According to this results we can say: Assuming  a one pool model with constant decomposition rate with only a fraction of the total carbon accessible for decomposition, the best fitting model has a decomposition rate of {\tt 2.3\%/d} where only {\tt 0.12\%} of the total carbon are accessible for decomposition.
%We can also check the estimated correlation between our two parameters. The high (anti)correlation indicates that it is difficult to identify the best model confidently.  
%
%The results of {\tt modFit} can be used for Bayesian parameter estimation with \FME's Monte Carlos Markow Chain function {\tt modMCMC}. In our example, we used the default values to avoid explainations about \FME, but there are many possibilities to influence the result which are briefly mentioned in the help {\tt ? modMCMC} and explained in more detail in \citet{Soetaert}.
...
% pairs(eCO2P1amcmc)
%@
%We can visualize the results of the Monte Carlos sampling also by estimating the effect on the results (at different times) with function {\tt sensRange} and plotting the results as a function of time.
%<<>>=
...
%  )
%@
%Note that the results depend not only on our equations, but also on the the other arguments of {\tt modMCMC} which for instance determine how many parameter combinations will be tested or how much they differ, or in general how well the parameter space has been probed.
%
%As a last example we try to further improve the fit of the model by allowing a second pool with its decomposition rate as second parameter. The role of the first parameter as constant decomposition rate for the first pool does not change.
%The second paramter of the last model becomed the third here and decides now how the total carbon is distributed between the two pools. 
%<<>>=
...
%summary(eCO2P2fit)
%@
%We see again a high correlation between the parameters, which indicats poor identifiability. 
%Numerical experiments (e.g. variation of the startvalues) also show that the algorithms occaisionally get lost in local minima, so that including more pools or fluxes does not seem to be a very good idea.
%Note however that although we probably can not hope to fit models that are structuarally more complex there are infinitly more ways to choose parameters. We could for instance define a time dependent decomposition rate whose shape depends on more than the one parameter we chose for a constant rate.
%
...
As before we wrapt it in a function that takes as arguments a set of parameters and returns the 
...
@
To create the cost function we reorganize the observed data. Note that we do not have standard deviations for the measurements, so the cost function will not have to weight the residuals. 
<<>>=
DataR14t=cbind(
