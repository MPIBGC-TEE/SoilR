...
\item 
	\label{independence} Make sure that the parameters can at least be tested independently by the optimization procedure. 
		...
		For n parameters the algorithms that we will use except n ranges and assume to be able to choose \emph{any} combination of parameters out of this n-dimensionalrectangular set to form a \emph{valid} model. E.g. an optimization algorithm that changes parameters independently should not accidentally create a model that breakes mass conservation.)
\end{enumerate}
We will start with the simplest model that fulfills conditions \enumref{identifiability} and \enumref{independence} and increase and extend it to improve the degree of accuracy \enumref{accuracy} until we are satisfied or can no lnger guarantee \enumref{identifiability}.
Note that the search for accurate and identifyable soil models is an open research question. The possible answers are limited only by your igenuity and the availability of data with the latter becoming obvious in our example.
