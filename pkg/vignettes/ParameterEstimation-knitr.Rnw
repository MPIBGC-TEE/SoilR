...
The first \FME function that we want to use is {\tt modFit}. A look at its documentation:
<<tidy=TRUE>>=
? modFit
@
suggests that we at least have to provide a function {\tt f} to be minimized a vector of start values for the parameters and in our case a lower limit for
